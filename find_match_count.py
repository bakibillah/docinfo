...
def update_searchpeoplefree_(single_match, rowid):
    try:
        with conn().cursor() as cursor:
            update = f'UPDATE `search_people2` SET `single_match` =%s WHERE `id` =%s;'
            result = cursor.execute(update, (single_match, rowid))
...
def select_all():
    try:
        with conn().cursor() as cursor:
            select_sql = "SELECT `id`, `match1`,`match2`,`match3`,`match4`,`match5`,`match6`,`match7`,`match8` FROM `docinfo_org`.`search_people2` where listofaddress2 is not null;"
...
#     n += 1
#     row_id = item['id']
#     d_location = item['dLocations']
#     if d_location is None:
#         continue
#
#     locations = d_location.split('|')
...
#
#     if listofaddress1 is not None:
#         listofaddress1_list = json.loads(listofaddress1)
#         address_list = [item_.split(',') for item_ in listofaddress1_list]
#         processed_addresses = []
#
...
#             print('match1: 0')
#             update_searchpeoplefree(match=0, match_list=None, num=1, row_id=row_id)
#         else:
#             print('match1: ', len(common_elements), list(common_elements))
#             update_searchpeoplefree(match=len(common_elements), match_list=json.dumps(list(common_elements)), num=1, row_id=row_id)
#
