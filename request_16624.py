...
command = "google-chrome --user-data-dir=$HOME/16624 --proxy-server=138.197.224.38:16624 --remote-debugging-port=16624 --remote-allow-origins=http://localhost:16624"
...
def get_cookie(chrome_, change_ip):
    while change_ip:
        burp0_url = "https://gridpanel.net/api/reboot?token=NjY3MCxiOGMzMTdiMi0yMTI4LTRjODctYTVkZC0yYjczZTIzN2JiZjk%3D"
...
while break_while:
    time.sleep(2)
    chrome = PyChromeDevTools.ChromeInterface(port=16624)
    ...
    for doc_id in all_doc_id:
        ...
        try:
            res = session.get(
                burp0_url,
                proxy="http://138.197.224.38:16624",
