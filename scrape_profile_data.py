...
def run_scraper(count_):
    command = "google-chrome --user-data-dir=$HOME/profile15582 --proxy-server=138.197.224.38:15582 --remote-debugging-port=15582 --remote-allow-origins=http://localhost:15582"
    chrome_process = subprocess.Popen(command, shell=True, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                                      stderr=subprocess.DEVNULL, close_fds=True)
    time.sleep(5)

    chrome = PyChromeDevTools.ChromeInterface(port=15582)
