...
def run_scraper(count_):
    command = "google-chrome --user-data-dir=$HOME/proxy33 --proxy-server=65.21.25.28:1037 --remote-debugging-port=9233 --remote-allow-origins=http://localhost:9233"
    ...
    while True:
        ...
        for doc_id in all_doc_id:
            ...
            print('-------------------------------------------------')
            try:
                event, messages = chrome.wait_event("Page.frameStoppedLoading", timeout=20)
                value = chrome.wait_event("Network.responseReceived", timeout=20)
